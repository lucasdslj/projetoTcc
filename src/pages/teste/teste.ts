...
import { mapStyle2 } from '../battle-screen/mapStyle';
import { mapStyle } from '../battle-screen/mapStyle';
...
export class TestePage {
  ...
  createMap(board, axisX, axisY, T) {
    ...
    var edgeBord = [];
    var position, marker, square, markerIcon, minZ, aux = board.length - 1, colorBoard = '#00BFFF', colorAxis = '#000000';
      ...
      if (axisY[1][j] == 0) {
        ...
        for (var i = 0; i < board.length; i++) {
          ...
          marker = new google.maps.Marker({
            ...
            label: { text: axisX[1][i].toString(), fontSize: '12px', fontWeight: '475' },
            icon: markerIcon
          });
        }
      }
